...
object ClimateService {
  ...
  def isClimateRelated(description: String): Boolean = {
    // Create a variable with the key words.
    val keywords = Set("global warming", "IPCC", "climate change")
    // Check if a key word is included is the sentence.
    keywords.exists(description.contains)
  }
  ...
  def parseRawData(list: List[(Int, Int, Double)]): List[Option[CO2Record]] = {
    // Map over this list.
    list.map {
      // Unpack each tuple into its three components.
      case (date, time, ppm) =>
        // Construct a new `CO2Record` instance from the raw data.
        val co2Record = CO2Record(date, time, ppm)
        // Check if the ppm value is valid. If so, wrap the `CO2Record` instance in a `Some`.
        // If the ppm value is invalid return None.
  ...
  def filterDecemberData(list: List[Option[CO2Record]]) : List[CO2Record] = {
    val filteredList = list.flatten.filter(_.month != 12)
    filteredList
  }
   ...
   */
  def getMinMax(list: List[CO2Record]): (Double, Double) = ((list.minBy(_.ppm).ppm, list.maxBy(_.ppm).ppm))
  ...
  def getDifference(list: List[Double]): Double = {
